...
export const LicenseTokenDialog = ({ open, onOpenChange }: LicenseDialogProps) => {
  ...
  const handleImportClick = async () => {
    ...
    console.log('[LicenseDialog] Starting license file import...');
    
    // Hide the overlay temporarily to allow native dialog to appear
    // Find and hide the Radix overlay element
    const hideOverlay = () => {
    ...
    // Hide overlay before opening native dialog
    hideOverlay();
    
    try {
      const result = await importLicenseFile();
      console.log('[LicenseDialog] Import result:', { success: result.success, error: result.error });
      ...
      if (result.success) {
        ...
        console.error('[LicenseDialog] Import failed:', result.error);
        setImportError(result.error);
      } else if (result.error === 'cancelled') {
        console.log('[LicenseDialog] User canceled import');
        // Don't show error for cancellation
      }
    } catch (error) {
      // Restore overlay
      showOverlay();
      console.error('[LicenseDialog] Unexpected error during import:', error);
