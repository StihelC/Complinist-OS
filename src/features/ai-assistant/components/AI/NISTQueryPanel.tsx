...
import { Send, Loader2, StopCircle, Trash2, Filter, ChevronDown, ChevronRight, Lightbulb } from 'lucide-react';
...
export function NISTQueryPanel() {
  ...
  const explanationMode = useNISTQueryStore((state) => state.explanationMode);
  ...
  const setExplanationMode = useNISTQueryStore((state) => state.setExplanationMode);
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between border-b px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <AIStatusIndicator />
          <Button
            variant={explanationMode === 'eli5' ? 'default' : 'ghost'}
            size="sm"
            onClick={() => setExplanationMode(explanationMode === 'eli5' ? 'standard' : 'eli5')}
            title={explanationMode === 'eli5' ? 'ELI5 Mode: Using simple analogies' : 'Standard Mode: Technical explanations'}
            data-testid="eli5-toggle"
          >
            <Lightbulb className={`w-4 h-4 mr-1 ${explanationMode === 'eli5' ? 'text-yellow-300' : ''}`} />
            ELI5
          </Button>
          <Button
            variant="ghost"
      ...
      <div className="border-t px-4 py-3 bg-gray-50">
        ...
        <div className="mt-2 flex items-center gap-3 text-xs text-gray-500">
          {explanationMode === 'eli5' && (
            <span className="flex items-center gap-1 text-amber-600" data-testid="eli5-mode-indicator">
              <Lightbulb className="w-3 h-3" />
              ELI5 Mode: Using simple analogies
            </span>
          )}
          {(selectedDocumentTypes.length > 0 || selectedFamilies.length > 0) && (
            <span>Active filters: {selectedDocumentTypes.length + selectedFamilies.length}</span>
          )}
        </div>
      </div>
    </div>
