...
import { Send, Loader2, StopCircle, Trash2, Filter, FileText, Library, Layers } from 'lucide-react';
...
const SEARCH_SCOPES: Array<{ value: SearchScope; label: string; icon: React.ReactNode; description: string }> = [
  { value: 'both', label: 'All Sources', icon: <Layers className="h-3 w-3" />, description: 'My docs + Compliance library' },
  { value: 'user', label: 'My Documents', icon: <FileText className="h-3 w-3" />, description: 'Only your uploaded docs' },
  { value: 'shared', label: 'Compliance Library', icon: <Library className="h-3 w-3" />, description: 'NIST, CMMC, RMF, etc.' },
];
...
export function UnifiedAIChat() {
  ...
  const searchScope = useNISTQueryStore((state) => state.searchScope);
  ...
  const setSearchScope = useNISTQueryStore((state) => state.setSearchScope);
  ...
  return (
    <div className="flex flex-col h-full bg-white">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SEARCH_SCOPES.map((scope) => (
                <Badge
                  key={scope.value}
                  variant={searchScope === scope.value ? 'default' : 'outline'}
                          ...
                          {entry.response.references.map((ref, refIndex) => (
                            <div key={refIndex} className="p-2 bg-white rounded border">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  {(ref as any).source === 'user_document' ? (
                              ...
                              {(ref as any).source === 'user_document' && (ref as any).filename && (
                                <div className="text-xs mt-1 opacity-75">{(ref as any).filename}</div>
                              )}
                              {ref.controlName && (
      ...
      <div className="border-t p-4">
        ...
        {(selectedDocumentTypes.length > 0 || selectedFamilies.length > 0 || searchScope !== 'both') ? (
          <div className="mt-2 text-xs text-gray-500 flex items-center gap-2">
            ...
            {selectedDocumentTypes.length + selectedFamilies.length > 0 && (
              <span>{selectedDocumentTypes.length + selectedFamilies.length} filter(s)</span>
            )}
          </div>
        ) : null}
      </div>
